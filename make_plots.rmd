...
```{r Store Seurat objects that are merged in a list for each dataset for easier access}
...
dir_test <- dir("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/Cardiac", recursive = T)

seurat_object_list <- list()
data_names <- c("Cardiac", "Endothelial", "pbmc_10x")#
for (j in data_names){
  if (j =="Cardiac"){
    sample_names <- paste(c("MF1701"), c(0,3:8), sep = "")
  } else if(j=="Endothelial"){
    sample_names=c("Brain","Colon","Heart","Kidney","Liver","Lung","muscle_EDL","muscle_Soleus","Small_Intestine","Spleen","Testis")
  } else if(j == "pbmc_10x"){
    sample_names <- paste(c("sample"), c(1:4), sep = "")
  }
  ...
  for ( i in sample_names){
    if (j %in% c("Endothelial", "pbmc_10x")){
      i <- paste("results_filtered_anno", i, sep = "/")
    }
  print(i)
  print(paste0("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/", j, "/alevin/"))
  alevin_raw <- import_alevin_mat(paste0("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/", j, "/alevin/"), i, "alevin/quants_mat_cols.txt")
  seurat_object_list[[paste(j,i, "alevin", sep = "__")]] <- CreateSeuratObject(alevin_raw)
  
  cellranger_raw <- Read10X(paste0("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/", j, "/cellranger/", i, "/outs/emptyDrops_feature_bc_matrix/"))
  seurat_object_list[[paste(j,i, "cellranger", sep = "__")]] <- CreateSeuratObject(cellranger_raw)
  
  if(j %in% c("Cardiac", "pbmc_10x")){
    print(paste0("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/", j, "/cellranger4/", i, "/outs/emptyDrops_feature_bc_matrix/"))
    if (j=="pbmc_10x"){
      i <- strsplit(i, "/")[[1]][2]
    }
    cellranger4_raw <- Read10X(paste0("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/", j, "/cellranger4/", i, "/outs/emptyDrops_feature_bc_matrix/"))
    seurat_object_list[[paste(j,i, "cellranger4", sep = "__")]] <- CreateSeuratObject(cellranger4_raw)
    print("cellranger5")
    cellranger5_raw <- Read10X(paste0("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/", j, "/cellranger5/", i, "/outs/emptyDrops_feature_bc_matrix/"))
    seurat_object_list[[paste(j,i, "cellranger5", sep = "__")]] <- CreateSeuratObject(cellranger5_raw)
    if (j=="pbmc_10x"){
      i <- paste("results_filtered_anno", i, sep = "/")
    }
  }
  print("kallisto")
  kallisto_raw <- Read10X(paste0("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/", j, "/kallisto/", i, "/counting/filtered_matrix_emptyDrops/"))
  seurat_object_list[[paste(j,i, "kallisto", sep = "__")]] <- CreateSeuratObject(kallisto_raw)
  print("starsolo")
  starsolo_raw <- Read10X(paste0("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/", j, "/starsolo/", i, "/Solo.out/Gene/filtered_matrix_emptyDrops/"))
  seurat_object_list[[paste(j,i, "starsolo", sep = "__")]] <- CreateSeuratObject(starsolo_raw)
}
}
saveRDS(seurat_object_list, "/media/Storage/additional_datasets/cellranger_4_5.rds")
...
create_plot_data <- function(data){
  ...
  colnames(data_melt) <- c("value", "L1", "Dataset", "Sample", "Mapper")
  ...
  levels(data_mean$Mapper) <- c("Alevin", "Cell Ranger", "Cell Ranger v4", "Cell Ranger v5", "Kallisto", "STARsolo")
  data_mean$Mapper <- factor(data_mean$Mapper, levels = c("Cell Ranger", "Cell Ranger v4", "Cell Ranger v5", "STARsolo", "Alevin", "Kallisto"))
  print(str(data_mean))
  return(data_mean)
}
...
cellranger4_raw <- Read10X("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/Cardiac/cellranger4/MF17010/outs/emptyDrops_feature_bc_matrix/")
    CreateSeuratObject(cellranger4_raw)
    cellranger5_raw <- Read10X("/media/Helios_scStorage/Ralf/Benchmark/MappedReads/Cardiac/cellranger5/MF17010/outs/emptyDrops_feature_bc_matrix/")
    CreateSeuratObject(cellranger5_raw)
...
Cluster together all 4 samples per tool with an anchoring to get a better clustering
```{r PBMC Integration}
library(tictoc)
#results_dir <- "/media/Storage/additional_datasets/plots/pbmc/"
results_dir <- "/media/Storage/additional_datasets/mouse_cell_atlas/"
tissue <- c("Heart","Kidney","Liver","Brain","Colon","Lung","muscle_EDL","muscle_Soleus","Small_Intestine","Spleen","Testis")
samples <- c("sample1","sample2","sample3","sample4","sample5","sample6","sample7","sample8")
sample_names <- apply(expand.grid(tissue,samples),1,paste,collapse="")

#sample_names <- c("sample1", "sample2","sample3", "sample4")
tools <- c("cellranger","starsolo","alevin","kallisto")#,"starsolo","alevin","kallisto"

#clustered_objects_unfiltered <- list()
clustered_objects <- list()
cluster_plot <- list()
#cluster_plot_unfiltered <- list()
tic()
for (k in tools){
 print(k)
  seurat_data <- list()
  for (j in sample_names){
    ...
    filtered_data <- filtered_data_seurat_obj_filtered_mouse_atlas[[paste(j,k,"results_filtered_anno",sep="__")]]
    filtered_data <- RenameCells(filtered_data,add.cell.id = paste(j,sep="_"))
    #colnames(filtered_data) <- paste(colnames(filtered_data),j,k,sep="_")
    seurat_data[[paste(k,j,sep="_")]] <- subset(filtered_data,subset=nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 10)
  }
  rm(filtered_data)
  rm(filtered_data_seurat_obj_filtered_mouse_atlas)
  # Data Integration
  seurat_data <- lapply(seurat_data,function(x) x <- SCTransform(x))

  seurat_features <- SelectIntegrationFeatures(object.list = seurat_data,nfeatures = 2500)
  seurat_data <- PrepSCTIntegration(object.list = seurat_data,anchor.features = seurat_features)

  seurat_anchors <- FindIntegrationAnchors(object.list = seurat_data,normalization.method = "SCT",anchor.features = seurat_features)
  rm(seurat_data)
  ...
  set_resolution <- 0.51
 #   if (k=="Cellranger"){
 #   set_resolution <- 0.125
 # }else if((k=="Star_solo") | (k=="Kallisto")){
 #   set_resolution <- 0.12
 # }else if (k=="Alevin"){
 #   set_resolution <- 0.12
#}
  seurat_integrated <- FindClusters(seurat_integrated,resolution = set_resolution)
  Project(seurat_integrated) <- k
  clustered_objects[[k]] <- seurat_integrated # Save object for further anaylsis

}

saveRDS(clustered_objects,"/media/ATLAS_NGS_storage/RalfSB/Results_Rosenthal/clustered_objects_05_06_20.Rds")
saveRDS(clustered_objects,paste0(results_dir,"pbmc_integrated.Rds"))
saveRDS(cluster_plot,paste0(results_dir,"pbmc_integrated_plots.Rds"))
#clustered_objects <- readRDS("/media/ATLAS_NGS_storage/RalfSB/Results_Rosenthal/clustered_objects_new_kallisto.Rds")


tools <- c("cellranger","starsolo","alevin","kallisto")#,"starsolo","alevin","kallisto"
cluster_plot <- list()
for (k in tools){
  cluster_plot[[k]] <- DimPlot(clustered_objects[[k]],reduction = "umap", label = T)
}


p_umap_pbmc_comb <- ggarrange(cluster_plot$cellranger,cluster_plot$starsolo,cluster_plot$alevin,cluster_plot$kallisto,ncol = 2, nrow = 2, labels = "AUTO")
ggsave("/media/Storage/additional_datasets/plots/pbmc/other/umap_combined.svg", p_umap_pbmc_comb, device = "svg",height = 10,width = 11)


DimPlot(clustered_objects[[k]],group.by = "orig.ident", label = T)
```



Cluster together all 11 samples per tool with an anchoring to get a better clustering
```{r Cardiac Integration}
...
tools <- c("Cellranger","Star_solo","Alevin","Kallisto")#


filtered_data_seurat_obj <- readRDS("/media/Storage/home_backup/data/imported_data_un_filtered.RDS")
filtered_data_seurat_obj <- filtered_data_seurat_obj[grep("__filtered",names(filtered_data_seurat_obj))]

seurat_data <- lapply(filtered_data_seurat_obj, function(x) {
