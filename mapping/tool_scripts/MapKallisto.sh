...
for sampledir in ${SAMPLEDIRS[*]}; do
	...
	echo "kallisto bus --index=$index --output-dir=${outFolderSample}${SAMPLENAME} --threads=$threads --technology=$chemistry $READ1 2> ${outFolderSample}${SAMPLENAME}_output_log.txt" >>  $outFolder"commands_kallisto.txt"
	#$READ2 
    SECONDS=0
	kallisto bus --index=$index --output-dir=${outFolderSample} --threads=$threads --technology=$chemistry $READ1 2>> ${outFolderSample}${SAMPLENAME}_output_log.txt

	#Barcode Correction
	echo "Barcode correction"
	if [[ $chemistry == "10xv3" ]]; then
		echo "bustools correct -w $whitelist -o ${outFolderSample}output.correct.bus ${outFolderSample}output.bus 2 >> ${outFolderSample}${SAMPLENAME}_output_correct_log.txt" >>  $outFolder"commands_kallisto.txt"
	bustools correct -w $whitelist -o ${outFolderSample}output.correct.bus ${outFolderSample}output.bus 2> ${outFolderSample}${SAMPLENAME}_output_correct_log.txt
	...
	echo "bustools sort -t $threads -o ${outFolderSample}output.corrected.sorted.bus ${outFolderSample}output.correct.bus" >>  $outFolder"commands_kallisto.txt"
	bustools sort -t $threads -o ${outFolderSample}output.corrected.sorted.bus ${outFolderSample}output.correct.bus
