...
#4 Filter by Empty Drops Method
    ...
    #    cp ${i}raw_feature_bc_matrix/features.tsv.gz ${i}filtered_matrix_emptyDrops/features.tsv.gz
    #done
    #
    #
    ## STARsolo
    ...
    for i in `ls -1d ${main_outpath}MappedReads/pbmc_10x/alevin-fry/results_filtered_anno/sample*/sample*_quant_res/`; do
      { time Rscript --vanilla ${github_path}mapping/post_mapping/filter_raw_emptyDrops.R ${i} $i "alevin-fry" "emptyDrops" $threads ; } 2> ${i}runtime_empty_drops.txt 
      #mv ${i}filtered_matrix_emptyDrops/features.tsv.gz ${i}filtered_matrix_emptyDrops/features_backup.tsv.gz &&\
      #cp ${i}raw/features.tsv.gz ${i}filtered_matrix_emptyDrops/features.tsv.gz
    done
