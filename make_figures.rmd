...
read_data <- function(dataset_m, tool_m, filtered_m=NULL, sample_m, m_gene_col=2){
  ...
  if (grepl("cellranger", tool_m)){
    ...
    print(paste0(path_in, "/",sample_m, "_quant_res/alevin/"))
    data <- Read10X(paste0(path_in, "/",sample_m, "_quant_res/alevin/"), gene.column = m_gene_col)
...
save_seurat_objects <- function(dataset, tools, filtered, set_gene_col=2){
  filtered_data <- list()
  seurat_list <- list()
  for (i in datasets){
    ...
    for (j in tools){
      ...
      for (k in filtered){
        ...
        for (l in sample_names){
          ...
          filtered_data[[id_name]] <- read_data(i,j,k,l,m_gene_col = set_gene_col)
...
datasets <- c("Endothelial", "Cardiac", "pbmc_10x", "HF")
datasets_nice <- c("Endothelial", "Cardiac", "PBMC", "HF")
...
saveRDS(pbmc_cardiac_endo, "/media/Storage/Benchmark/pbmc_cardiac_endo_hf.rds")
#pbmc_cardiac_endo <- readRDS("/media/Storage/Benchmark/pbmc_cardiac_endo.rds")
```
...
get_runtimes <- function(dataset_m){
  ...
  runtimes <- lapply(tools, function(x) {
    ...
    if (x == "alevin-fry" | dataset_m == "HF" && x=="kallisto"){
      runtime_paths <- runtime_paths[grep("runtime_sec", runtime_paths)]
    }else if (x %in% c("cellranger6", "starsolo")){
      runtime_paths <- runtime_paths[grep("runtime.txt", runtime_paths)]
    }
    
    print(runtime_paths)
    run <- lapply(runtime_paths, function(p) {
...
all_runtimes <- list(PBMC = get_runtimes(datasets[1]),
     Endothelial = get_runtimes(datasets[2]),
     Cardiac = get_runtimes(datasets[3]),
     HF = get_runtimes(datasets[4]))
...
runtime_mean$Dataset <- factor(runtime_mean$Dataset, levels = datasets_nice)
...
all_mapping_rates <- list(PBMC = get_mapping_rate(datasets[1]),
     Endothelial = get_mapping_rate(datasets[2]),
     Cardiac = get_mapping_rate(datasets[3]),
     HF = get_mapping_rate(datasets[4]))
...
all_genes_mouse <- read.table(paste0(github_path, "mapping/all_genes_mouse.tsv"),
                                     header = T,  colClasses = c(rep("character",4)))

all_genes_human <- read.delim(paste0(github_path, "mapping/all_genes_human.tsv"),
                              header = T,  colClasses = c(rep("character",4)))
...
saveRDS(gene_counts, "/media/Helios_scStorage/Ralf/Benchmark/plots_revision/intermediate_data/suppl_figure3gene_counts.rds")
gene_counts <- readRDS("/media/Helios_scStorage/Ralf/Benchmark/plots_revision/intermediate_data/suppl_figure3gene_counts.rds")
...
#saveRDS(pbmc_cardiac_endo, "/media/Storage/Benchmark/pbmc_cardiac_endo_sctransform.rds")
#pbmc_cardiac_endo <- readRDS("/media/Storage/Benchmark/pbmc_cardiac_endo_sctransform.rds")
...
#saveRDS(integrated_data, "/media/Storage/Benchmark/integrated_data_HF.rds")
integrated_data<- readRDS("/media/Storage/Benchmark/integrated_data_pbmc_cardiac.rds")
names(integrated_data$PBMC) <- tools_nice
names(integrated_data$Cardiac) <- tools_nice
names(integrated_data$HF) <- tools_nice
#saveRDS(integrated_data["Endothelial"], "/media/Storage/Benchmark/integrated_data_endothelial.rds")
...
integrated_data <- readRDS("/media/Storage/Benchmark/integrated_data_HF.rds")
...
for (m in seq_along(marker_genes)){
  p_marker<- DotPlot(integrated_data$HF$`Cell Ranger 6`, features = marker_genes[[m]], assay = "RNA") +
    theme(axis.text.x = element_text(angle = 60,vjust=+1.05 ,hjust = +1.1))
  ggsave(paste0("/media/Helios_scStorage/Ralf/Benchmark/plots_revision/tmp/marker_genes/HF/", names(marker_genes)[m] , ".svg"), p_marker, device = "svg")
}
...
#### Create plots ####
figure6 <- list()
integrated_data <- readRDS("/media/Storage/Benchmark/integrated_data_pbmc_cardiac_new.rds")
saveRDS(integrated_data, "/media/Storage/Benchmark/integrated_data_pbmc_cardiac_new.rds")
...
# 
# 
# ggsave("/media/Helios_scStorage/Ralf/Benchmark/plots_revision/tmp/barcodes_per_tool_pbmc.svg")
# #### Cardiac ####
# 
# scina_cardiac <- scina_classi(integrated_data$Cardiac, cell_type_genes_cardiac)
# #saveRDS(cell_classification_scina, "/media/Storage/additional_datasets/intermediate_data/figure6/suppl_figure6_scina_classification_cardiac.rds")
...
#         panel.grid.major.x = element_blank())
# 
# ggsave("/media/Helios_scStorage/Ralf/Benchmark/plots_revision/tmp/barcodes_per_tool_cardiac.svg")
...
pbmc_integrated <- readRDS("/media/Storage/Benchmark/integrated_data_pbmc.rds")
