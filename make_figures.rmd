...
packages <- c("reshape2", "Seurat", "ggplot2", "ggpubr", "Matrix", 
              "jsonlite", "egg", "svglite", "SCINA", "ggalluvial", "dplyr")
...
#### !!! Please define the path to your downloaded Github directory and the output directory 
github_path="/path/to/main/GithubRepository/BenchmarkAlignment/"
main_outpath="/path/to/main/output/" #same main_outpath as in shell scripts
setwd(main_outpath)

dir.create("plots_revision")
tmp/upset/
...
map_data$Dataset <- factor(map_data$Dataset)
levels(map_data$Dataset) = datasets_nice
...
if (datasets %in% c("Cardiac","Endothelial")){
...
cell_type_genes_pbmc <- list(
  ...
  "T-cell CD8+" = c("Cd8a","Cd8b","cd27","Themis")
)
...
```{r Precision and Recall Main Fig 4 Suppl Fig 6}
...
ggsave("plots_revision/Suppl_Figure_f6_recall_prec.svg", height = 14, width = 12)



suppl_figure_6_prec_recall <- list()
if (datasets %in% "Cardiac"){
suppl_figure_6_prec_recall[["p_cardiac_prec"]] <- p_cardiac_prec + ggtitle("Cardiac") + theme(axis.text.y = element_blank(),
                                         axis.text.x = element_blank())
suppl_figure_6_prec_recall[["p_cardiac_recall"]] <- p_cardiac_recall + theme(axis.text.y = element_blank())
}
if (datasets %in% "PBMC"){
suppl_figure_6_prec_recall[["p_pbmc_prec"]] <- p_pbmc_prec + theme(axis.text.x = element_blank()) + ylab("Precision") + ggtitle("PBMC")
suppl_figure_6_prec_recall[["p_pbmc_recall"]] <- p_pbmc_recall + ylab("Recall")
}
p_prec_recall <- ggpubr::ggarrange(plotlist = suppl_figure_6_prec_recall,
...
ggsave("plots_revision/tmp/Figure_4_A.svg", p_f1, device = "svg", height = 8, width = 10)
...
ggsave("plots_revision/Suppl_Figures/Suppl_Figure_5.svg", device = "svg", width = 18, height = 12)
...
#p_deg_overlap
ggsave("plots_revision/Figure_4.svg", p_deg_overlap, height = 6, width = 8)
...
#mt_content <- read.table("additional_datasets/mt_content_read_counts.tsv")
...
ggsave("plots_revision/tmp/suppl_figure8_umap_new_cells.png", cluster_plot2[[1]], device="png", width = 8, height = 5)
...
ggsave("plots_revision/Suppl_figure_4.png", p_heat_arranged, device = "png", height = 10, width = 16)
